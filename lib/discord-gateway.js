...
/**
 * High-volume inbound dispatches we ignore anyway. Their frames are not
 * logged, to keep protocol diagnostics (handshake, presence, close) readable.
 */
const QUIET_DISPATCHES = new Set([
    "GUILD_CREATE", "GUILD_UPDATE", "GUILD_DELETE",
    "GUILD_MEMBER_ADD", "GUILD_MEMBER_REMOVE", "GUILD_MEMBERS_CHUNK",
    "GUILD_ROLE_CREATE", "GUILD_ROLE_UPDATE", "GUILD_ROLE_DELETE",
    "PRESENCE_UPDATE", "CHANNEL_CREATE", "CHANNEL_UPDATE", "CHANNEL_DELETE",
]);
...
class DiscordGateway extends EventEmitter {
     ...
     * @param {string} [reason] Optional close reason text from the server.
     * @private
     */
    #onClose(code, reason) {
        this.#stopHeartbeat();
        ...
        if (code === 4004) {
            ...
            this.emit("fatal", new Error(
                `gateway authentication failed (close code 4004${reason ? `: ${reason}` : ""}) — check your token`
            ));
            return;
        }
